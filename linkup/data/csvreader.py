...
def process_fitteum_csv(
    ...
    if "equipment" in df.columns:
        bodyweight_df = df[
            df["equipment"].astype(str).str.lower().str.strip() == "body weight"
        ].copy()
    else:
    ...
    instruction_cols = [
        col for col in bodyweight_df.columns if "instruction" in col.lower()
    ]
    ...
    def analyze_exercise_details(row: pd.Series) -> pd.Series:
        ...
        if any(
            word in search_text
            for word in ["cardio", "jump", "run", "burpee", "jacks", "rope"]
        ):
            category = "유산소"
        elif any(
            word in search_text
            for word in ["stretch", "stretching", "yoga", "flexibility", "warm-up"]
        ):
            category = "스트레칭"
        else:
            category = "근력"
    ...
    # 카테고리와 난이도(숫자형) 적용
    bodyweight_df[["category", "difficulty_level"]] = bodyweight_df.apply(
        analyze_exercise_details, axis=1
    )
    ...
    if "target" in bodyweight_df.columns:
        ...
        bodyweight_df["contraindications"] = (
            bodyweight_df["bodyPart"] if "bodyPart" in bodyweight_df.columns else ""
        )
    ...
    bodyweight_df[requested_columns].to_csv(
