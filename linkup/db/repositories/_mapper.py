...
from linkup.db.constants import (
    BodyPart, ExerciseCategory, Scene, JobType, Gender, Goal, SessionStatus,
    parse_csv, join_csv,
)
...
def _enum_csv(items) -> str:
    ...
    return join_csv([getattr(i, "value", i) for i in items])
...
# User_Profile
# ------------------------------------------------------------------
def row_to_user_profile(r) -> UserProfile:
    return UserProfile(
        id=r["id"],
        nickname=r["nickname"],
...
def user_profile_to_params(p: UserProfile) -> dict:
    ...
    return {
        ...
        "job_type": p.job_type.value if isinstance(p.job_type, JobType) else p.job_type,
        "goals": _enum_csv(p.goals),
        "goal_duration_weeks": p.goal_duration_weeks,
        "weekly_frequency": p.weekly_frequency,
        "pain_points": _enum_csv(p.pain_points),
...
# Exercise_Library
# ------------------------------------------------------------------
def row_to_exercise(r) -> ExerciseLibraryItem:
...
# Daily_Log
# ------------------------------------------------------------------
def row_to_daily_log(r) -> DailyLog:
...
def daily_log_to_params(log: DailyLog) -> dict:
    fatigue = {
...
# Workout_Session
# ------------------------------------------------------------------
def row_to_session(r) -> WorkoutSession:
...
# Workout_History
# ------------------------------------------------------------------
def row_to_history(r) -> WorkoutHistory:
